...
class LocoManipPolicyXR(LocoManipPolicy):
    ...
    def _handle_xr_buttons(self, tele_data):
        ...
        if self.stand_command[0, 0]:
            ...
            # WebXR gamepad axis convention: pushing the stick forward (away
            # from the operator) reports ly = -1, so this must be negated to
            # make forward-on-stick mean forward-on-robot.
