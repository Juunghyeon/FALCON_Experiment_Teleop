...
if __name__ == '__main__':
    ...
    parser.add_argument('--upper-ref-only', action = 'store_true',
                         help = 'Do not drive arm motors directly (no rt/lowcmd / rt/arm_sdk writes). '
                                'Instead publish the dual-arm IK solution over rt/teleop_upper_ref for a '
                                'FALCON sim2real policy process to consume as ref_upper_dof_pos. '
                                'Use this when a sim2real policy (loco_manip.py) is already controlling the robot.')
    ...
    try:
        ...
        if args.upper_ref_only:
            # A sim2real policy process owns rt/lowcmd; this process only computes
            # IK and publishes ref_upper_dof_pos, so no arm_sdk / debug mode is needed.
            pass
        elif args.motion:
        ...
        if args.upper_ref_only:
            from teleop.utils.teleop_upper_ref_pub import TeleopUpperRefPublisher
            upper_ref_publisher = TeleopUpperRefPublisher()
        ...
        while not STOP:
            ...
            if arm_ctrl is not None:
                ...
                # upper-ref-only mode: no local motor feedback: let solve_ik seed
                # from its own last solution (current_lr_arm_motor_q=None path).
            ...
            if arm_ctrl is not None:
                ...
                upper_ref_publisher.publish(sol_q)
            ...
            if args.record:
                ...
                left_arm_state  = current_lr_arm_q[:7] if current_lr_arm_q is not None else np.zeros(7)
                right_arm_state = current_lr_arm_q[-7:] if current_lr_arm_q is not None else np.zeros(7)
